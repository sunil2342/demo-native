...
public extension Notification.Name {
    static let SourceInterfaceIgnoredMethodInvocation = Notification.Name("SourceInterfaceIgnoredMethodInvocationNotification")
    static let SourceInterfaceWillMethodInvocation = Notification.Name("SourceInterfaceWillMethodInvocationNotification")
}

private let identifier = "/* SourceInterface */"

open class SourceInterface: NSObject {
                   ...
                   var list = Optional(methodList)
                {
                    ...
                    while list?.pointee.name != nil {
                        ...
                        guard let selector = list?.pointee.name,
                              ...
                              let signature = String(cString: types, encoding: .utf8)
                        else {
...
extension SourceInterface: WKScriptMessageHandler {
    ...
    public func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        guard let target = target,
              ...
              let bridge = bridges.first(where: { $0.jsSelector == message.name })
        else {
            return
        }
        ...
        notificationCenter.post(name: .SourceInterfaceWillMethodInvocation, object: nil, userInfo: userInfo)
        ...
        if ignoreMethodCallWhenReceivedNull {
            for arg in args {
                if arg is NSNull {
                    ...
                    notificationCenter.post(name: .SourceInterfaceIgnoredMethodInvocation, object: nil, userInfo: userInfo)
                    return
                }
            }
        }
